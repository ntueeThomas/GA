...
int main(){
	...
	for(int c = 0; c < 1; c++){
		...
		double generation = 0;
		double preAvgFitness = 0;
		double curAvgFitness = 0;
		double variance = 0;
		double standard_deviation = 0;
		//cout<<"***************** generation: "<<generation<<" *******************"<<endl;
		evaluate(A, F, curTotalFitness);
		preAvgFitness = curTotalFitness/n;
		for(int i = 0; i < n; i++){
			variance += pow(F.at(i) - preAvgFitness, 2);
		}
		variance /= n;
		...
		while(preTotalFitness != curTotalFitness){
			...
			*/
			...
			replace(A, B);	
			evaluate(A, F, curTotalFitness);
			curAvgFitness = curTotalFitness/n;
			//cout<<"standard_deviation: "<<standard_deviation<<endl;
			cout<<"selection intensity: "<<(curAvgFitness - preAvgFitness)/standard_deviation<<endl;
			preAvgFitness = curAvgFitness;
			variance = 0;
			for(int i = 0; i < n; i++){
				variance += pow(F.at(i) - preAvgFitness, 2);
			}
