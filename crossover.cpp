...
void uniformCrossover(vector<vector<bool> > &B, MyRand &myRand){
	...
	for(int i = 0; i < num; i++){
		...
		for(int j = 0; j < B.at(0).size(); j++){
			...
			if(myRand.flip(0.5)){
				//cout<<"order"<<endl;
				temp.at(0).at(j) = B.at(pair_index.at(2*i)).at(j);
				temp.at(1).at(j) = B.at(pair_index.at(2*i+1)).at(j);
			}
			else{
				//cout<<"reverse order"<<endl;
				temp.at(0).at(j) = B.at(pair_index.at(2*i+1)).at(j);
				temp.at(1).at(j) = B.at(pair_index.at(2*i)).at(j);
			}
			
		}
		B.at(pair_index.at(2*i)) = temp.at(0);
		B.at(pair_index.at(2*i+1)) = temp.at(1);
		//cout<<"****************************************"<<endl;
		//printAGene(pair_index.at(2*i), B);
		//printAGene(pair_index.at(2*i+1), B);
		//cout<<"***************************** END ****************************"<<endl;
...
void tnSelectionWoR(vector<vector<bool> > &A, vector<vector<bool> > &B, vector<double> &F, MyRand &myRand){
